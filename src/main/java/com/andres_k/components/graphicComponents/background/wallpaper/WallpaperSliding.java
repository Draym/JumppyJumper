...
public class WallpaperSliding extends Wallpaper {
    ...
    public WallpaperSliding(AnimatorController animator, float speed, boolean horizontal, boolean moveRight) throws SlickException {
        this(animator, speed, horizontal, moveRight, -1);
    }

    public WallpaperSliding(AnimatorController animator, float speed, boolean horizontal, boolean moveRight, int limitLine) throws SlickException {
        super(animator, 0, 0);
        this.speed = speed;
        ...
        float nbY = (limitLine > 0 ? limitLine : ((float)WindowConfig.get().getWindowSizes(EnumWindow.GAME).getV2() / (float)animator.currentAnimation().getCurrentFrame().getHeight()) + 1.0f);
        ...
        if (this.animator != null && this.activated) {
            ...
            try {
                for (int i = 0; i < this.nbImage; ++i) {
                    ...
                    if (this.moveRight) {
                        if (this.horizontal) {
                            this.animator.draw(g, this.x + incrX, this.y + incrY);
                        } else {
                            this.animator.draw(g, this.x + incrX, this.y + incrY - animator.currentAnimation().getCurrentFrame().getHeight());
                        }
                    } else {
