...
public class GameController extends WindowController {
    ...
    public void init() throws SlickException {
        ...
        this.backgroundManager.addComponent(EBackground.MAP_1, new WallpaperSliding(ResourceManager.get().getBackgroundAnimator(EBackground.BACKGROUND_BLOCK_1), 10, true, true));
    }
        ...
        if (this.running) {
            ...
            if (winners > 0) {
                ...
                if (GameConfig.mode == EMode.ONLINE) {
                    ...
                    CentralTaskManager.get().sendRequest(TaskFactory.createTask(this.location, ELocation.GAME_GUI_PanelQuit_Details, new Pair<>(ETaskType.ADD, ElementFactory.createText("Good Job ! ", ColorTools.get(ColorTools.Colors.GUI_GREEN), EFont.MODERN, 50, 60, 10))));
                    CentralTaskManager.get().sendRequest(TaskFactory.createTask(this.location, ELocation.GAME_GUI_PanelQuit_Details, new Pair<>(ETaskType.ADD, ElementFactory.createText(winners + " slimes survived", ColorTools.get(ColorTools.Colors.GUI_BLUE), EFont.MODERN, 20, 20, 50))));
                    CentralTaskManager.get().sendRequest(TaskFactory.createTask(this.location, ELocation.GAME_GUI_PanelQuit_Details, new Pair<>(ETaskType.ADD, ElementFactory.createText("you get " + GameObjectController.get().bonusPoint + " super coin!", ColorTools.get(ColorTools.Colors.GUI_YELLOW_BLAND), EFont.MODERN, 20, 20, 70))));
                }
            } else {
                CentralTaskManager.get().sendRequest(TaskFactory.createTask(this.location, ELocation.GAME_GUI_PanelQuit_Details, new Pair<>(ETaskType.ADD, ElementFactory.createText("Game Over !", ColorTools.get(ColorTools.Colors.GUI_RED), EFont.MODERN, 50, 60, 10))));
                CentralTaskManager.get().sendRequest(TaskFactory.createTask(this.location, ELocation.GAME_GUI_PanelQuit_Details, new Pair<>(ETaskType.ADD, ElementFactory.createText("none of your slimes survived", ColorTools.get(ColorTools.Colors.GUI_ORANGE), EFont.MODERN, 20, 20, 50))));
                CentralTaskManager.get().sendRequest(TaskFactory.createTask(this.location, ELocation.GAME_GUI_PanelQuit_Details, new Pair<>(ETaskType.ADD, ElementFactory.createText("you get " + GameObjectController.get().bonusPoint + " super coin!", ColorTools.get(ColorTools.Colors.GUI_YELLOW_BLAND), EFont.MODERN, 20, 20, 70))));
                CentralTaskManager.get().sendRequest(TaskFactory.createTask(this.location, ELocation.GAME_GUI_PanelQuit_Details, new Pair<>(ETaskType.ADD, ElementFactory.createText("Total score : " + GameObjectController.get().bonusPoint, ColorTools.get(ColorTools.Colors.GUI_YELLOW_BLAND), EFont.MODERN, 20, 20, 70))));
            }
            this.running = false;
        }
    }
