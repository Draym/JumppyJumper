...
public class BodyAnimation {
    ...
    public void draw(Graphics g, int currentFrame, boolean haveToFlip, float posX, float posY, float rotateAngle) {
        if (currentFrame < this.bodies.size()) {
            this.bodies.get(currentFrame).draw(g, haveToFlip, posX, posY, rotateAngle);
        }
    }
