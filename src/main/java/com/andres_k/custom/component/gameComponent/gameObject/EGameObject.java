...
public enum EGameObject {
    ...
    PORTAL_REPULSE("PORTAL_REPULSE", PLATFORM.getValue()),
    PORTAL_ATTRACT("PORTAL_ATTRACT", FILMY.getValue()),
    ...
    public static Object[] getAllFrom(EGameObject search) {
        ArrayList<EGameObject> values = new ArrayList<>();
    ...
    public boolean isIn(EGameObject[] dirs) {
        for (EGameObject dir : dirs) {
            boolean result = this.isIn(dir);
            if (result) {
                return true;
            }
        }
